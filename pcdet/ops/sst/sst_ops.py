...
def scatter_v2(feat, coors, mode, return_inv=True, min_points=0, unq_inv=None, new_coors=None, fusion=False):
    ...
    if unq_inv is None and min_points > 0:
        ...
        if fusion:
            new_coors, unq_inv = torch.unique_consecutive(coors, return_inverse=True, return_counts=False, dim=0)
        else:
...
def get_flat2win_inds(batch_win_inds, voxel_drop_lvl, drop_info, debug=True):
    ...
    for dl, v in enumerate(drop_info): # dl: short for drop level
...
def get_inner_win_inds(win_inds):
    ...
    sort_inds, order = win_inds.sort() #sort_inds is like [0,0,0, 1, 2,2] -> [0,1, 2, 0, 0, 1] 오름차순 정렬해서 결과와 순서를 얻음, 복셀이 속한 윈도우의 인덱스가 정렬됨
    roll_inds_left = torch.roll(sort_inds, -1) # [0,0, 1, 2,2,0] 왼쪽으로 한칸씩 순환이동, 각 복셀의 왼쪽에 있는 복셀의 윈도우 인덱스를 얻음

    diff = sort_inds - roll_inds_left #[0, 0, -1, -1, 0, 2] 현재 보클의 윈도우 인덱스와 왼쪽 보클의 윈도우 인덱스 간의 차이를 계산합니다. 차이가 0인 경우, 같은 윈도우에 속한 보클을 의미하며, 차이가 1인 경우, 다른 윈도우에 속한 보클을 의미
    end_pos_mask = diff != 0 # 윈도우 변경 위치(차이가 0이 아닌 위치)를 표시하는 마스크를 생성

    bincount = torch.bincount(win_inds) # 텐서 내 각 윈도우의 복셀 수
    # assert bincount.max() <= max_tokens
    unique_sort_inds, _ = torch.sort(torch.unique(win_inds)) # : win_inds에서 고유한 윈도우 인덱스 목록을 얻고 정렬, 이것은 각 윈도우의 순서
    num_tokens_each_win = bincount[unique_sort_inds] #[3, 1, 2] 각 윈도우에 속한 보클 수를 bincount로부터 가져옴

    template = torch.ones_like(win_inds) #[1,1,1, 1, 1,1] # win_inds와 동일한 크기의 1로 이루어진 텐서를 생성
    template[end_pos_mask] = (num_tokens_each_win-1) * -1 #[1,1,-2, 0, 1,-1] 윈도우 변경 위치에 해당하는 template 값을 갱신

    inner_inds = torch.cumsum(template, 0) #[1,2,0, 0, 1,0] template을 누적 합(cumulative sum)하여 각 복셀의 윈도우 내 상대적 위치 정보를 얻음
    inner_inds[end_pos_mask] = num_tokens_each_win #[1,2,3, 1, 1,2]  윈도우 변경 위치에 해당하는 inner_inds 값을 복셀 수로 갱신
    inner_inds -= 1 #[0,1,2, 0, 0,1] 모든 값에 1을 빼줌으로써 상대적 위치 정보를 0부터 시작하도록 조정

    #recover the order
    inner_inds_reorder = -torch.ones_like(win_inds) # win_inds와 동일한 크기의 -1로 이루어진 텐서를 생성
    inner_inds_reorder[order] = inner_inds # order를 사용하여 inner_inds를 원래 순서로 복구
...
def get_window_coors(coors, sparse_shape, window_shape, do_shift):
    # 복셀 좌표를 입력으로 받고, 윈도우 단위로 분할하여 각 복셀이 어떤 윈도우에 속하는지 계산
    ...
    if do_shift: # 쉬프트 윈도우, 시프트된 윈도우의 크기를 나타내며, 시프트가 적용되면 복셀이 중심에서 어떤 정도 떨어져 있는지를 나타냄

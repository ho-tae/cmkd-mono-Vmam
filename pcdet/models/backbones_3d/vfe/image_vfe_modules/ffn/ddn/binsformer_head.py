...
from depth.models.utils.transformer import PureMSDEnTransformer, PixelTransformerDecoder
from depth.utils.position_encoding import SinePositionalEncoding
...
import numpy as np
from depth.models.losses.celoss import CrossEntropyLoss
...
class BinsFormerDecodeHead(DepthBaseDecodeHead):
    ...
    def __init__(self,
        ...
        self.transformer_encoder = PureMSDEnTransformer(transformer_encoder)
        self.positional_encoding = SinePositionalEncoding(positional_encoding["num_feats"])
        ...
        if self.classify is True:
            self.loss_class = CrossEntropyLoss(loss_class)
        ...
        transformer_decoder.pop('type')
        self.transformer_decoder = PixelTransformerDecoder(**transformer_decoder)
    ...
    def forward(self, inputs):
        ...
        for item_bin, pred_logit, pred_class in \
            ...
            if self.binsformer is False:
                ...
                pred_logit1 = pred_logit.softmax(dim=1)
                pred_depth = torch.sum(pred_logit1 * centers, dim=1, keepdim=True)
        ...
        return pred_depths, pred_logit, pred_classes

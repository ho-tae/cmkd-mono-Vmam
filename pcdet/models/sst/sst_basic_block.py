...
from pcdet.ops.sst.sst_ops import flat2window, window2flat
...
class WindowAttention(nn.Module):
    ...
    def forward(self, feat_2d, pos_dict, ind_dict, voxel_drop_level, key_padding_dict, drop_info, cross_att=False):
        '''
        Args:
        ...
        if cross_att:
            ...
            top_feat_3d_dict = flat2window(top_feats, voxel_drop_level, ind_dict, drop_info)
...
class EncoderLayer(nn.Module):
    ...
    def forward(
        ...
        src2 = self.win_attn(src, pos_dict, ind_dict, voxel_drop_level, key_padding_mask_dict, drop_info, cross_att) #[N, d_model]
        if cross_att:
...
class BasicShiftBlock(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        # ModuleList로부터 EncoderLayer에 접근하여 각 마지막 선형 레이어의 편향을 설정
        #for encoder_layer in self.encoder_list:
        #    found_linear = False
        #    # EncoderLayer 내에 있는 모든 모듈을 확인
        #    encoder_layer.linear2.bias.requires_grad = False
    ...
    def forward(
        ...
        for i in range(2):
            ...
            if using_checkpoint:
                ...
                if not cross_atten:
                    output = layer(output, pos_dict, ind_dict, voxel_drop, key_mask_dict, drop_info, cross_att=False)
                else:
                    if i==0:
                        output = layer(output, pos_dict, ind_dict, voxel_drop, key_mask_dict, drop_info, cross_att=True)
                    else:
